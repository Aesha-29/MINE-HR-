        ...
        <div className="lm-container lm-fade">
            <div className="lm-page-header">
                ...
                <div className="tabs-container">
                    <button className={`tab-button ${tab === "regular" ? "active" : ""}`} onClick={() => setTab("regular")}>
                        <Calendar size={14} /> Regular Leaves
                        {pending.length > 0 && <span className="ml-1 bg-indigo-100 text-indigo-700 text-xs px-1.5 py-0.5 rounded-full">{pending.length}</span>}
                    </button>
                    <button className={`tab-button ${tab === "short" ? "active" : ""}`} onClick={() => setTab("short")}>
                        <Clock size={14} /> Short Leaves
                        {shortLeaves.length > 0 && <span className="ml-1 bg-indigo-100 text-indigo-700 text-xs px-1.5 py-0.5 rounded-full">{shortLeaves.length}</span>}
                    </button>
                    <button className={`tab-button ${tab === "payouts" ? "active" : ""}`} onClick={() => setTab("payouts")}>
                        <Banknote size={14} /> Payouts
                        {payouts.length > 0 && <span className="ml-1 bg-indigo-100 text-indigo-700 text-xs px-1.5 py-0.5 rounded-full">{payouts.length}</span>}
                    </button>
                </div>
            </div>
                        ...
                        <>
                            {shortLeaves.length === 0 ? (
                                <div className="lm-card text-center py-12">
                                    <CheckCircle size={48} color="#10b981" className="mx-auto mb-4" />
                                    <h3 className="text-gray-800 font-semibold mb-2">All caught up!</h3>
                                    <p className="text-gray-500">No pending short leave requests.</p>
                                </div>
                            ) : shortLeaves.map(r => (
                                <div className="lm-card" key={r.id}>
                                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "1rem", flexWrap: "wrap" }}>
                        ...
                        <>
                            {payouts.length === 0 ? (
                                <div className="lm-card text-center py-12">
                                    <CheckCircle size={48} color="#10b981" className="mx-auto mb-4" />
                                    <h3 className="text-gray-800 font-semibold mb-2">All caught up!</h3>
                                    <p className="text-gray-500">No pending leave payout requests.</p>
                                </div>
                            ) : payouts.map(r => (
                                <div className="lm-card" key={r.id}>
                                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "1rem", flexWrap: "wrap" }}>
